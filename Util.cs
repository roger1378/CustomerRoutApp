...
internal class Util
{
    ...
    internal string GetNextRouteExcluded(Dictionary<string, int> routes, string endValueKey, string exclude)
    {
        foreach (var item in routes)
        {
            if (item.Key.Substring(0, 1) == endValueKey && item.Key != exclude)
            {
                return item.Key;
            }
        }
